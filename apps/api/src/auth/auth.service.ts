...
export class AuthService {
  ...
  async register(email: string, password: string, confirmPassword: string) {
    ...
    await this.analyticsService.track({
      ...
      metadata: { emailDomain: emailDomain(user.email) },
    });
    return this.genericAuthEmailResponse();
  }
  ...
  async requestPasswordReset(email: string) {
    ...
    const cooldownMinutes = this.configService.getOrThrow<number>(
      "FIGCONTROL_PASSWORD_RESET_RESEND_COOLDOWN_MINUTES",
    );
  ...
  private async sendEmailVerification(userId: string, email: string) {
    ...
    const ttlMinutes = this.configService.getOrThrow<number>(
      "FIGCONTROL_EMAIL_VERIFICATION_TOKEN_TTL_MINUTES",
    );
  ...
  private async sendPasswordReset(userId: string, email: string) {
    const token = createOneTimeToken();
    const ttlMinutes = this.configService.getOrThrow<number>(
      "FIGCONTROL_PASSWORD_RESET_TOKEN_TTL_MINUTES",
    );
  ...
  private async hasRecentEmailVerificationToken(
    userId: string,
  ...
  private async hasRecentPasswordResetToken(
    userId: string,
    cooldownMinutes: number,
  ): Promise<boolean> {
